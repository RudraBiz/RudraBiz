...
export default async function InvoiceDetailPage({
  ...
  const invoice = await prisma.invoice.findFirst({
    ...
    include: {
      customer: true,
      lines: { include: { revenueAccount: true }, orderBy: { sortOrder: "asc" } },
  ...
  const recordPaymentWithIds = recordPayment.bind(null, slug, invoice.id);
  const voidWithIds = voidInvoice.bind(null, slug, invoice.id);

  return (
    <div className="flex min-h-full flex-col">
      <TopNav />
      <main className="mx-auto w-full max-w-2xl flex-1 px-6 py-10">
        <Link href={`/companies/${slug}/invoices`} className="text-sm text-muted hover:text-ink">
          ← Invoices
        </Link>
        ...
        <div className="mt-4 flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">
              {invoice.customer.name} · {invoice.invoiceDate.toISOString().slice(0, 10)}
        ...
        {invoice.notes && <p className="mt-3 text-sm">{invoice.notes}</p>}
            ...
            <tbody>
              {invoice.lines.map((line) => (
                <tr key={line.id} className="border-b border-border last:border-b-0">
                  ...
                  <td className="px-5 py-3 text-muted">{line.revenueAccount.name}</td>
        ...
        {canRecordPayment && invoice.status !== "VOID" && remaining > 0.001 && (
          <div className="mt-6 rounded-lg border border-border bg-surface p-5">
            ...
            <form action={recordPaymentWithIds} className="mt-3 flex flex-wrap items-end gap-3">
              ...
              <div className="flex-1 min-w-[180px]">
                <label className="mb-1 block text-xs font-medium">Deposit to</label>
                <select
        ...
        {canVoid && invoice.status !== "VOID" && invoice.payments.length === 0 && (
